  ...
  <div class="footer-area footer-left mdui-col-sm-4">
    <div class="footer-icons">
      <?php if ($this->options->footer && in_array('showrss', $this->options->footer)): ?>
        <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple" mdui-tooltip="{content: 'RSS'}" mdui-menu="{target: '#rss'}"><i class="mdui-icon typecho-material-theme-icons">&#xe90e;</i></button>
      ...
      <?php if ($this->options->email): ?>
        <a href="mailto:<?php $this->options->email(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple" mdui-tooltip="{content: '邮箱'}"><i class="mdui-icon typecho-material-theme-icons">&#xe90f;</i></button>
        </a>
      <?php endif; ?>
      <?php if ($this->options->github): ?>
        <a href="https://github.com/<?php $this->options->github(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple icons" mdui-tooltip="{content: 'GitHub'}"><i class="mdui-icon typecho-material-theme-icons">&#xe910;</i></button>
        </a>
      <?php endif; ?>
      <?php if ($this->options->twitter): ?>
        <a href="https://twitter.com/<?php $this->options->twitter(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple icons" mdui-tooltip="{content: 'Twitter'}"><i class="mdui-icon typecho-material-theme-icons">&#xe911;</i></button>
        </a>
      <?php endif; ?>
      <?php if ($this->options->facebook): ?>
        <a href="https://www.facebook.com/<?php $this->options->facebook(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple icons" mdui-tooltip="{content: 'Facebook'}"><i class="mdui-icon typecho-material-theme-icons">&#xe912;</i></button>
        </a>
      <?php endif; ?>
      <?php if ($this->options->weibo): ?>
        <a href="<?php $this->options->weibo(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple icons" mdui-tooltip="{content: '微博'}"><i class="mdui-icon typecho-material-theme-icons">&#xe913;</i></button>
        </a>
      <?php endif; ?>
      <?php if ($this->options->netease_music): ?>
        <a href="<?php $this->options->netease_music(); ?>" target="_blank">
          <button class="mdui-text-color-white mdui-btn mdui-btn-icon mdui-ripple icons" mdui-tooltip="{content: '网易云音乐'}"><i class="mdui-icon typecho-material-theme-icons">&#xe914;</i></button>
        </a>
      <?php endif; ?>
    </div>
  </div>
...
<?php if ($this->options->footer && in_array('showscrolltop', $this->options->footer)): ?>
  <button class="mdui-fab mdui-fab-fixed mdui-ripple mdui-color-pink-accent mdui-fab-hide" id="scrolltop"><i class="mdui-icon typecho-material-theme-icons">&#xe915;</i></button>
<?php endif; ?>
</body>
<?php $this->footer(); ?>
</html>
